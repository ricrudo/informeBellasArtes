...
from app.domain.models import AtendidosCasosEspeciales

import json

'''
{"nombre_persona": null, "detalles": null, "remision": null, "observaciones": null, "name_file": null}
'''
...
def new_entry_user(initform:dict):
    ...
    index_used = []
    error = []
    for index_entry, content in initform['section6_7'].items():
        if index_entry == 'activateQuestionsection6_7':
            continue
        index_used.append(int(index_entry))
        if len(content) != 5:
            error.append(f'Error: se han enviado {len(content)} datos. Se esperaban 5 datos.')
            continue
        if not all([x in content for x in (['nombre_persona', 'detalles', 'remision', 'observaciones', 'name_file'])]):
            error.append(f'Los keys no corresponden a los que recibe la base de datos.')
            continue
        
        content['name_file'] = filesManager.saveFile('section6_7', index_entry, initform)
        content['user'] = initform['user']
        content['period_spam'] = initform['period_spam']
        content['index_entry'] = int(index_entry)
        _send_to_DB(content)
    db_manager._delete_entries(index_used, initform['user'], initform['period_spam'], AtendidosCasosEspeciales)
    db.session.close()
    if error:
        return error
    return 'ok'
...
def _create_entry(form:dict):
    entry = AtendidosCasosEspeciales(
        id_person = form['user'],
...
def _send_to_DB(content:dict):
    entry = db_manager._check_entry(content, AtendidosCasosEspeciales)
    if entry:
