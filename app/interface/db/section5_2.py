...
'''
{"entidad": null, "actividades": null, "tipo_vinculo": null, "beneficiarios": null}
'''
...
def new_entry_user(initform:dict):
    ...
    index_used = []
    error = []
    for index_entry, content in initform['section5_2'].items():
        if index_entry == 'activateQuestionsection5_2':
            continue
        index_used.append(int(index_entry))
        if len(content) != 4:
            error.append(f'Error: se han enviado {len(content)} datos. Se esperaban 4 datos.')
            continue
        if not all([x in content for x in (['entidad', 'actividades', 'tipo_vinculo', 'beneficiarios'])]):
        ...
        _send_to_DB(content)
    db_manager._delete_entries(index_used, initform['user'], initform['period_spam'], GestionConvenios)
    db.session.close()
    if error:
        return error
    return 'ok'
...
def _send_to_DB(content:dict):
    entry = db_manager._check_entry(content, GestionConvenios)
    if entry:
