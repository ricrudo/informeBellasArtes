...
from app.domain.models import AtendidosBajoRendimiento
...
def new_entry_user(initform:dict):
    ...
    index_used = []
    error = []
    for index_entry, content in initform['section6_2'].items():
        if index_entry == 'activateQuestionsection6_2':
            continue
        ...
        content['name_file'] = filesManager.saveFile('section6_2', index_entry, initform)
        content['user'] = initform['user']
        content['period_spam'] = initform['period_spam']
        content['index_entry'] = int(index_entry)
        _send_to_DB(content)
    db_manager._delete_entries(index_used, initform['user'], initform['period_spam'], AtendidosBajoRendimiento)
    db.session.close()
    if error:
        return error
    return 'ok'
...
def _create_entry(form:dict):
    entry = AtendidosBajoRendimiento(
        id_person = form['user'],
...
def _send_to_DB(content:dict):
    entry = db_manager._check_entry(content, AtendidosBajoRendimiento)
    if entry:
