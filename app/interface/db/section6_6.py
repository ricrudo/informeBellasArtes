...
from app.domain.models import DesercionGraduacion
...
def new_entry_user(initform:dict):
    ...
    index_used = []
    error = []
    for index_entry, content in initform['section6_6'].items():
        if index_entry == 'activateQuestionsection6_6':
            continue
        ...
        _send_to_DB(content)
    db_manager._delete_entries(index_used, initform['user'], initform['period_spam'], DesercionGraduacion)
    db.session.close()
    if error:
        return error
    return 'ok'
...
def _send_to_DB(content:dict):
    entry = db_manager._check_entry(content, DesercionGraduacion)
    if entry:
